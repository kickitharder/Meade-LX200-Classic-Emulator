...
bool guideBuddy(char c) {
  ...
  switch (c) {
    ...
    case 'v':
      ...
      break;
    case '!': 
      resp[0] = c = (pulseTimer > 0) + '0';
      break;
    case '%':
      pulseTimer = 0;
      break;
    default:
      return false;           // Not a GuideBuddy command
      break;
  }
  updateCoords();
...
void setObjAZ() {             //  :Szddd*mm#  :Szddd*mm*ss#
  ...
  if (ee.align == 'L') return;
  if (buf[5] == '\xDF') buf[5] = '*';
  if (sscanf(buf, "Sz%03d*%02d:%02d", &d, &m, &s) <= 1) return;
...
void setDate() {              //  :SCMM/DD/YY#
  ...
  if(sscanf(buf, "SC%02d/%02d/%02d", &m, &d, &y) == 3) {
    ...
    mth = m;
    yr = y;
    sendMsg("1Updating        planetary data. ");
    if (!test) delay(5000);
    memset(resp, ' ', 32);
  }
}
...
void setTime() {              //  :SLHH:MM:SS#
  int h, m, s;
  if(sscanf(buf, "SL%02d:%02d:%02d", &h, &m, &s) == 3) {
    if((h < 24) && (m < 60) && (s < 60)) {
      updateTime();
      hr = h;
      min = m;
